...
<section class="content" id="dog-breed">
	<div class="container">
	<button class="button1" id="button1"><i class="material-icons addIcon">add</i>Add New Dog Breed</button>
<input type="text" class="search" placeholder="Search For Dog Breed" id="breed-search" list="breed-list">
...
function showBreed() {
    ...
    if ($result->num_rows > 0) {
        ...
        if($page==1){
        	...
        	echo '<a href="SideBar_Breed_Dog.php?page=' . ($page-1) . '">&lt;</a>';
        }
        for ($i = 1; $i <= $total_pages; $i++) {
		    if ($i == $page) {
		        echo '<a href="SideBar_Breed_Dog.php?page=' . $i . '" class="page-active">' . $i . '</a>';
		    } else {
		        echo '<a href="SideBar_Breed_Dog.php?page=' . $i . '">' . $i . '</a>';
		    }
	}
		if($page == $total_pages){
	...
	<div class="modal-content">
      ...
      <iframe name="hiddenFrame" class="hide"></iframe>
		<form id="breedForm" action="SideBar_Breed-Add-Breed.php" method="post" target="hiddenFrame" enctype="multipart/form-data">
			...
			<div class="tab">
				<input type="hidden" placeholder="Name..." name="type" value="Dog">
...
// Check if the current URL includes "SideBar_Breed_Dog.php?page=" or "SideBar_Breed_Cat.php?page="
if (currentUrl.includes("SideBar_Breed_Dog.php?page=") || currentUrl.includes("SideBar_Breed_Cat.php?page=")) {
  // Get the page number from the URL
  var pageNumber = currentUrl.split("?page=")[1];

  // Get the dropdown button element
  var dropdownBtn = document.getElementById("dropdown-btn-id");

  // Add the "active" class to the dropdown button
