...
<link rel="stylesheet" type="text/css" href="../Seller/css/SellerStyle.css">
<style type="text/css">
	body{
		overflow-x: hidden;
	}
...
if (file_exists('../Seller/pet_images/' . $row['pet_image'])) {
    $imageSrc2 = '../Seller/pet_images/' . $row['pet_image'];
}

$videoData = base64_encode($row['video']);
  $videoSrc = "data:video/mp4;base64," . $videoData;
  if (file_exists('../Seller/pet_videos/' . $row['video'])) {
    $videoSrc = '../Seller/pet_videos/' . $row['video'];
   }
 ...
 if ($row2['sellerID'] !== NULL) {
	...
	elseif (file_exists('../Seller/seller_images/' . $row3['image'])) {
	    $imageSrc6 = '../Seller/seller_images/' . $row3['image'];
}
}
 elseif($row2['shopID']!==NULL){
	...
	elseif (file_exists('../Seller/pet_shop_images/' . $row3['image'])) {
	    $imageSrc6 = '../Seller/pet_shop_images/' . $row3['image'];
}
 }
...
for ($i = 1; $i <= 6; $i++) {
    ...
    if (isset($row['img' . $i]) && file_exists('../Seller/pet_images/' . $row['img' . $i])) {
        $imageSrc = '../Seller/pet_images/' . $row['img' . $i];
?>
...
for ($i = 1; $i <= 6; $i++) {
   ...
   if (isset($row['video']) && file_exists('../Seller/pet_videos/' . $row['video'])) {
   		echo "<span class='dot' onclick='currentSlide($i)'></span>";
   	}?> 
</div>
