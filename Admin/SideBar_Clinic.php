...
function showClinic_Pending() {
    ...
    if ($result->num_rows > 0) {
        ...
        foreach ($rows as $row) {
            ...
            echo '<a href="SideBar_Clinic-Profile.php?admin=yes&id=' . $row['clinicID'] . '" target="_self"><span class="material-symbols-outlined" id="card-button">open_in_new</span></a>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
        }
...
$sql = "SELECT v.vetID,v.name,v.image,v.phone,v.email,c.name AS cname,v.ic,v.apc FROM vet v,clinic c WHERE v.clinicID=c.clinicID  AND v.ic NOT LIKE 'P.%' AND v.ic NOT LIKE 'F.%' AND v.ic NOT LIKE 'B.%' AND v.ic NOT LIKE 'C.%' ORDER BY vetID ";
    ...
    if ($result->num_rows > 0) {
        ...
        if (file_exists('../Clinic/vet_images/' . $image)) {
            $imageSrc2 = '../Clinic/vet_images/' . $image;
        }
        }
        else{
          $gender=$ic[-1];
...
function sendApprovalEmail(email, name) {
    var xhr = new XMLHttpRequest();
    xhr.open("GET", '../Email/Email_Success.php?email=' + email + '&name=' + name, true);
    xhr.send();
}
function sendRejectEmail(email, name,reason) {
    var xhr = new XMLHttpRequest();
    console.log("reason"+reason);
    xhr.open("GET", '../Email/Email_Reject.php?email=' + email + '&name=' + name +'&reason='+ reason, true);
    xhr.send();
}
